...
import { loadReader } from "./reader.mts";
import { uniqueEntries } from "./manifest.mts";
...
function createDecoder({ RSCache, IndexType, ConfigType, ModelGroup }) {
  ...
  async function terrainAsset(cache, regionId) {
    ...
    const tileBrightness = (x, y) => {
      ...
      const slopeBrightness =
        Math.trunc((256 * (-50 * normalX - 50 * normalY - 10 * normalZ)) / Math.trunc((lightMagnitude * 768) / 256)) +
        96;
      const setting = (tile) => tile.settings ?? 0;
      const occlusion =
        (setting(localTile(x - 1, y)) >> 2) +
        (setting(localTile(x, y - 1)) >> 2) +
        (setting(localTile(x + 1, y)) >> 3) +
        (setting(localTile(x, y + 1)) >> 3) +
        (setting(localTile(x, y)) >> 1);
    ...
    const overlayDefinition = async (id) => {
      if (!overlays.has(id)) {
        ...
        for (let offset = 0; offset < content.length; ) {
          ...
          if (opcode === 0) break;
          if (opcode === 1) {
            rgb = (content[offset] << 16) | (content[offset + 1] << 8) | content[offset + 2];
            offset += 3;
      ...
      for (let y = 0; y < 64; y++) {
        ...
        for (let sampleX = Math.max(0, x - 4); sampleX <= Math.min(63, x + 5); sampleX++)
          for (let sampleY = Math.max(0, y - 4); sampleY <= Math.min(63, y + 5); sampleY++) {
            ...
            const hueMultiplier = Math.max(
              1,
              Math.floor(
                512 *
                  (definition.saturation / 256) *
                  Math.min(definition.lightness / 256, 1 - definition.lightness / 256),
              ),
            );
            hue += definition.hue;
        ...
        if (count && multiplier)
          blendedUnderlays[x][y] = packHsl((hue * 256) / multiplier, saturation / count, lightness / count);
      }
    ...
    for (let x = 0; x < 64; x++)
      for (let y = 0; y < 64; y++) {
        ...
        const cornerBrightness = [
          tileBrightness(x, y),
          tileBrightness(x + 1, y),
          tileBrightness(x + 1, y + 1),
          tileBrightness(x, y + 1),
        ];
        // rs-map-viewer's default `smoothTerrain: false` uses the blended
        // south-west underlay value for every corner of a tile.
        const underlayHsl =
          tileColors.underlay == null
            ? null
            : cornerBrightness.map((brightness) => adjustUnderlayBrightness(tileColors.underlay, brightness));
        const overlayHsl =
        ...
        const vertexForShape = (value) => {
          ...
          const points = {
            1: [0, 0, cornerHeight[0]],
            2: [0.5, 0, (cornerHeight[1] + cornerHeight[0]) / 2],
            3: [1, 0, cornerHeight[1]],
            4: [1, 0.5, (cornerHeight[1] + cornerHeight[2]) / 2],
            5: [1, 1, cornerHeight[2]],
            6: [0.5, 1, (cornerHeight[2] + cornerHeight[3]) / 2],
            7: [0, 1, cornerHeight[3]],
            8: [0, 0.5, (cornerHeight[3] + cornerHeight[0]) / 2],
            9: [0.5, 0.25, (cornerHeight[1] + cornerHeight[0]) / 2],
            10: [0.75, 0.5, (cornerHeight[2] + cornerHeight[1]) / 2],
            11: [0.5, 0.75, (cornerHeight[2] + cornerHeight[3]) / 2],
            12: [0.25, 0.5, (cornerHeight[3] + cornerHeight[0]) / 2],
            13: [0.25, 0.25, cornerHeight[0]],
            14: [0.75, 0.25, cornerHeight[1]],
            15: [0.75, 0.75, cornerHeight[2]],
            16: [0.25, 0.75, cornerHeight[3]],
          }[v];
          const colourCorners = {
            1: [0],
            2: [0, 1],
          ...
          for (const vertex of [a, b, c]) {
            ...
            const hsl =
              layerHsl == null
                ? null
                : Math.trunc(
                    vertex.colourCorners.reduce((sum, corner) => sum + layerHsl[corner], 0) /
                      vertex.colourCorners.length,
                  );
  ...
  async function sceneAssets(cache, regionId, assets, scenes, hooks) {
    ...
    const objectTerrain = (location, width, length) => {
      const x = location.position.localX,
        y = location.position.localY,
        plane = location.position.height;
      ...
      return {
        height,
        elevation: height / 128 - terrainBaseHeight / 128,
        heightAt,
        varied: samples.some((value) => value !== height),
      };
    };
    ...
    for (const sourceLocation of locations) {
      const location = applySceneTouchups(regionId, sourceLocation, scenes, hooks);
      ...
      for (const modelOrientation of modelOrientations) {
        ...
        const contourKey = needsContour
          ? `:${location.position.localX}:${location.position.localY}:${location.position.height}`
          : "";
        ...
        placements.push({
          assetId,
          x: location.position.localX,
          y: location.position.localY,
          plane: terrain.elevation,
          width,
          height,
        });
      }
    ...
    return {
      ...
      compiledAssets: {},
      placements: [
        { assetId: `scene-${regionId}-terrain`, x: 0, y: 0, plane: 0 },
        ...placements.filter((placement) => available.has(placement.assetId)),
      ],
    };
  }
  ...
  function payload(group, includeFace = (model, face) => true, definition) {
    ...
    const positions = [],
      ...
      animayaScales = [],
      alphas = [],
      alphaGroups = (model.faceLabelsAlpha ?? []).map(() => []),
      vertexGroups = (model.vertexGroups ?? []).map(() => []);
    for (let face = 0; face < model.faceVertexIndices1.length; face++) {
      ...
      [model.faceVertexIndices1[face], model.faceVertexIndices2[face], model.faceVertexIndices3[face]].forEach(
        (source, corner) => {
          const index = positions.length / 3;
          positions.push(
            model.vertexPositionsX[source] / 128,
            -model.vertexPositionsY[source] / 128,
            -model.vertexPositionsZ[source] / 128,
          );
          indices.push(faceIndices[corner]);
          ...
          uvs.push(
            model.faceTextureUCoordinates?.[face]?.[corner] ?? 0,
            model.faceTextureVCoordinates?.[face]?.[corner] ?? 0,
          );
          textureIds.push(faceTexture);
          for (let groupIndex = 0; groupIndex < vertexGroups.length; groupIndex++)
            if (model.vertexGroups[groupIndex]?.includes(source)) vertexGroups[groupIndex].push(index);
          for (let groupIndex = 0; groupIndex < alphaGroups.length; groupIndex++)
            if (model.faceLabelsAlpha[groupIndex]?.includes(face)) alphaGroups[groupIndex].push(index);
        },
      );
    }
    return {
    ...
    for (const id of [...new Set(sequenceIds.filter((value) => Number.isInteger(value) && value >= 0))]) {
      ...
      if (sequence.animMayaID != null && sequence.animMayaID !== -1) {
        ...
        for (let frame = 0; frame < sequence.animMayaEnd; frame++) {
          frameDef.framemap.animayaSkeleton
            .getAllBones()
            .forEach((bone, index) => frameDef.method727(frame, bone, index, frameDef.field1257));
          matrices.push(
            frameDef.framemap.animayaSkeleton
              .getAllBones()
              .map((bone) => Array.from(bone.method687(frameDef.field1257).matrixVals)),
          );
        }
  ...
  async function itemModels(cache, item, models) {
    ...
    for (const key of ["maleModel0", "maleModel1", "maleModel2"]) {
      if (item[key] >= 0) {
        await modelAsset(cache, item[key], models);
        ids.push(item[key]);
      }
    }
    return ids;
  ...
  async function spotAnimAsset(cache, id, models) {
    ...
    result.spotAnim = {
      ...
      resizeX: definition.resizeX ?? 128,
      resizeY: definition.resizeY ?? 128,
  ...
  return async function decode({
    ...
    try {
      ...
      const assets = [];
      for (const npcAsset of uniqueEntries(CACHE_ASSETS.npcs).map(([, entry]) => entry)) {
        ...
        const npcGroup = new ModelGroup(
          npcModelIds.map((id) => models.get(id)),
          false,
        );
        const clickboxFilter =
          hooks.clickboxFilter?.[npcAsset.id] ??
          (npcAsset.clickbox
            ? (model, face) => (model.faceAlphas?.[face] ?? 0) === npcAsset.clickbox.faceAlpha
            : undefined);
      ...
      for (const modelAssetDefinition of uniqueEntries(CACHE_ASSETS.models).map(([, entry]) => entry)) {
      ...
      const spotAnimIds = uniqueEntries(CACHE_ASSETS.spotAnims)
        .map(([, entry]) => entry)
        .map(({ id }) => id);
      ...
      const playerPoseMap = Object.fromEntries(
        Object.entries(semanticPoseMap).map(([index, animation]) => [index, animation.id]),
      );
      const playerItemAssets = {};
      let sharedPlayerAnimations;
      for (const [itemName, itemDefinition] of uniqueEntries(CACHE_ASSETS.items)) {
        ...
        if (!itemIds.length) {
          ...
          playerPayload = {
            ...
            color: 0xffffff,
            animations: {},
            poseMap: playerPoseMap,
          };
        } else {
          const group = new ModelGroup(
            itemIds.map((id) => models.get(id)),
            false,
          );
          playerPayload = await attachTextures(cache, payload(group, undefined, item));
          const itemAnimations = await animations(cache, group, [
            ...new Set([
              ...Object.values(playerPoseMap),
              ...Object.values(CACHE_ASSETS.playerAnimations).map(({ id }) => id),
            ]),
          ]);
          ...
          // item payload. Maya sequences have no raw frame map, so they stay
          // attached to the item that supplied their animation data.
          const shared = {};
          const itemSpecific = {};
        ...
        // Item IDs are the stable contract used by SDK equipment definitions.
        // The normalized name lets gameplay use the manifest's readable key.
        playerItemAssets[`item:${item.id}`] = assetId;
        playerItemAssets[itemKey(itemName)] = assetId;
      }
      for (const [name, { id }] of Object.entries(CACHE_ASSETS.items))
        playerItemAssets[itemKey(name)] = playerItemAssets[`item:${id}`];
      let sharedAssets;
      ...
      const sceneRegionIds = uniqueEntries(CACHE_ASSETS.regions)
        .map(([, entry]) => entry)
        .map(({ id }) => id);
      for (const regionId of sceneRegionIds) assets.push(await terrainAsset(cache, regionId));
      const scenes = {};
      for (const regionId of sceneRegionIds) {
        const recipe = await sceneAssets(cache, regionId, assets, sceneTouchups, hooks);
        const compiled = compileScene(recipe, assets);
        assets.push(...compiled);
        recipe.compiledAssets = Object.fromEntries(
          compiled.map((asset) => [
            asset.id.endsWith("-terrain") ? "terrain" : asset.id.endsWith("-transparent") ? "transparent" : "opaque",
            asset.id,
          ]),
        );
      ...
      for (let index = assets.length - 1; index >= 0; index--) {
        ...
        if ([...sceneInputPrefix].some((prefix) => id.startsWith(prefix)) && !id.includes("-compiled-"))
      ...
      const soundEffectIds = new Set(Object.values(CACHE_ASSETS.sounds).map(({ id }) => id));
