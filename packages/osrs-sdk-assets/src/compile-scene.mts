...
// Arena scenes use one region-sized chunk and one mesh per material.
const CHUNK_SIZE = 64;
const CHUNK_GRID = 64 / CHUNK_SIZE;

type Output = Required<
  Pick<CacheRenderStaticChunk, "positions" | "indices" | "colors" | "faceColors" | "alphas" | "uvs" | "textureIds">
> & { textures: NonNullable<CacheRenderPayload["textures"]> };
const output = (): Output => ({
  positions: [],
  indices: [],
  colors: [],
...
/** Bake the placement transform applied by CacheRenderSceneModel.
...
export function compileScene(recipe: Recipe, sourceAssets: SourceAsset[]) {
  ...
  const terrainOutput = new Map<string, Output>(),
    opaque = new Map<string, Output>(),
    transparent = new Map<string, Output>();
  ...
  for (const rawPlacement of recipe.placements) {
    const placement =
      rawPlacement.assetId.endsWith("-terrain") || !recipe.mirrorY
        ? rawPlacement
    ...
    for (let index = 0; index < indices.length; index += 3) {
      const transparentFace = [indices[index], indices[index + 1], indices[index + 2]].some(
        (vertex) => (source.alphas?.[vertex] ?? 0) !== 0,
      );
      ...
      const chunkX = terrain
        ? Math.floor(
            (transform(
              source.positions[indices[index] * 3],
              source.positions[indices[index] * 3 + 1],
              source.positions[indices[index] * 3 + 2],
            )[0] +
              transform(
                source.positions[indices[index + 1] * 3],
                source.positions[indices[index + 1] * 3 + 1],
                source.positions[indices[index + 1] * 3 + 2],
              )[0] +
              transform(
                source.positions[indices[index + 2] * 3],
                source.positions[indices[index + 2] * 3 + 1],
                source.positions[indices[index + 2] * 3 + 2],
              )[0]) /
              (CHUNK_SIZE * 3),
          )
        : Math.floor(placement.x / CHUNK_SIZE);
      const chunkY = terrain
        ? Math.floor(
            (transform(
              source.positions[indices[index] * 3],
              source.positions[indices[index] * 3 + 1],
        ...
        : Math.floor(placement.y / CHUNK_SIZE);
    ...
    for (const target of [...terrainOutput.values(), ...opaque.values(), ...transparent.values()])
